...
export type IndustryVideoV5Props = {
  imageUrl: string;
  audioUrl: string;
...
export const industryVideoV5DefaultProps: IndustryVideoV5Props = {
  ...
  prompt: "",
  hookText: "AUTOMATED CASH DRAWER",
  bodyText: "",
  themeColor: "",
  industry: "Restaurant",
};
    ...
    <AbsoluteFill style={{ overflow: "hidden" }}>
      ...
      <Img
        ...
        style={{
          ...
          objectFit: "cover",
          opacity: 0.65,
          filter: "brightness(0.5) contrast(1.05) blur(3px)",
          transform: `scale(${zoom}) translate(${driftX}px, ${driftY}px)`,
        }}
      />
      <AbsoluteFill style={{ background: "linear-gradient(180deg, rgba(8,11,17,0.3) 0%, rgba(8,11,17,0.65) 100%)" }} />
    </AbsoluteFill>
  );
};
...
const CyberHero: React.FC<{ imageUrl: string; accentColor: string }> = ({ imageUrl, accentColor }) => {
  const frame = useCurrentFrame();
  
  // Neon flicker effect
  const flicker = Math.sin(frame * 0.8) > 0.5 ? 1 : 0.82;
  const scanlineY = (frame * 12) % 800;

  return (
    <Interactive.Div
      name="Cyber Hero Frame"
      style={{
        position: "relative",
        width: 820,
        height: 820,
        ...
        boxShadow: `0 0 30px ${accentColor}, inset 0 0 20px ${accentColor}`,
        opacity: flicker,
      }}
    >
      <Img
      ...
      <div style={{ position: "absolute", top: 15, left: 15, width: 30, height: 30, borderTop: "4px solid #fff", borderLeft: "4px solid #fff" }} />
      <div style={{ position: "absolute", top: 15, right: 15, width: 30, height: 30, borderTop: "4px solid #fff", borderRight: "4px solid #fff" }} />
      <div style={{ position: "absolute", bottom: 15, left: 15, width: 30, height: 30, borderBottom: "4px solid #fff", borderLeft: "4px solid #fff" }} />
      <div style={{ position: "absolute", bottom: 15, right: 15, width: 30, height: 30, borderBottom: "4px solid #fff", borderRight: "4px solid #fff" }} />
    </Interactive.Div>
  );
};
...
const parseRichText = (text: string, accentColor: string) => {
  ...
  return parts.map((part, index) => {
    if (part.startsWith("[strong]") && part.endsWith("[/strong]")) {
      return (
        <span key={index} style={{ color: "#ffffff", fontWeight: 900, textShadow: `0 0 12px ${accentColor}, 0 0 25px ${accentColor}` }}>
          {part.slice(8, -9)}
        </span>
      );
    }
...
export const calculateIndustryVideoV5Metadata: CalculateMetadataFunction<IndustryVideoV5Props> = async ({ props }) => {
...
export const IndustryVideoV5: React.FC<IndustryVideoV5Props> = ({
  imageUrl,
  audioUrl,
  backgroundMusicUrl,
  text,
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg, fontFamily, overflow: "hidden" }}>
      {/* Full-screen Cyber Tech Glowing Outer Border */}
      <div
        style={{
          position: "absolute",
          top: 30,
          ...
          borderRadius: 36,
          boxShadow: `0 0 25px ${accentColor}22, inset 0 0 25px ${accentColor}22`,
          pointerEvents: "none",
          zIndex: 10,
        }}
      />
      ...
      >
        ...
        <CyberHero imageUrl={imageUrl} accentColor={accentColor} />
