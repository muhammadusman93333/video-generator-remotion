...
export type IndustryVideoV3Props = {
  imageUrl: string;
  audioUrl: string;
...
export const industryVideoV3DefaultProps: IndustryVideoV3Props = {
...
const GlowOrb: React.FC<{ accentColor: string }> = ({ accentColor }) => {
  const frame = useCurrentFrame();
  const pulse = Math.sin(frame * 0.05) * 0.08 + 1.0;
  const opacity = Math.sin(frame * 0.04) * 0.08 + 0.32;
  return (
    <div
      style={{
        position: "absolute",
...
const CardHero3D: React.FC<{ imageUrl: string; accentColor: string }> = ({ imageUrl, accentColor }) => {
  const frame = useCurrentFrame();
  
  // Dynamic sways (3D perspective floating effect)
  const rotateX = Math.sin(frame * 0.045) * 6.0;
  const rotateY = Math.cos(frame * 0.035) * 8.0;
  const translateY = Math.sin(frame * 0.05) * 15;

  return (
    <Interactive.Div
      name="3D Card Hero Container"
      style={{
        position: "relative",
        perspective: 1200,
        zIndex: 2,
    ...
    >
      <Interactive.Div
        ...
        style={{
          ...
          boxShadow: `0 40px 90px rgba(0,0,0,0.7), 0 0 60px ${accentColor}33`,
...
export const calculateIndustryVideoV3Metadata: CalculateMetadataFunction<IndustryVideoV3Props> = async ({ props }) => {
...
export const IndustryVideoV3: React.FC<IndustryVideoV3Props> = ({
  imageUrl,
  audioUrl,
  backgroundMusicUrl,
  text,
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg, fontFamily, overflow: "hidden" }}>
      ...
      >
        ...
        <CardHero3D imageUrl={imageUrl} accentColor={accentColor} />
