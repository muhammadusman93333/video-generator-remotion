...
export type IndustryVideoV4Props = {
  imageUrl: string;
  audioUrl: string;
...
export const industryVideoV4DefaultProps: IndustryVideoV4Props = {
  ...
  prompt: "",
  hookText: "INVOICE AND INVENTORY",
  bodyText: "",
  themeColor: "",
  industry: "Restaurant",
};
...
const CollageHero: React.FC<{ imageUrl: string; accentColor: string }> = ({ imageUrl, accentColor }) => {
  const frame = useCurrentFrame();
  
  // Spring entry for Card 1
  const card1Scale = interpolate(frame, [5, 18], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.out(Easing.back(1.5)),
  });

  // Spring entry for Card 2
  const card2Scale = interpolate(frame, [12, 25], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.out(Easing.back(1.5)),
  });
  ...
  return (
    ...
    >
      ...
      {/* Primary Card (Large, shifted right-down) */}
      <div
        style={{
          position: "absolute",
          right: 20,
          ...
          transform: `scale(${card1Scale}) translateY(${sway}px)`,
          zIndex: 2,
        }}
      >
        <Img
      ...
      {/* Secondary Accent Card (Small, shifted left-up) */}
      <div
        style={{
          position: "absolute",
          left: 40,
          ...
          transform: `scale(${card2Scale}) translateY(${-sway}px)`,
          zIndex: 3,
        }}
      >
        <Img
...
export const calculateIndustryVideoV4Metadata: CalculateMetadataFunction<IndustryVideoV4Props> = async ({ props }) => {
...
export const IndustryVideoV4: React.FC<IndustryVideoV4Props> = ({
  imageUrl,
  audioUrl,
  backgroundMusicUrl,
  text,
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg, fontFamily, overflow: "hidden" }}>
      ...
      >
        ...
        <CollageHero imageUrl={imageUrl} accentColor={accentColor} />
